...
const OperationsAgent = () => {
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      // Mock data so UI can still be previewed
      setData({
        metrics_analyzed: { temp: 92, cpu_load: 96, network_bandwidth_gbps: 9.8, power_usage: 85 },
        anomalies: ["Temperature surged to 92°C (Threshold 85°C)", "CPU load bottleneck at 96%"],
            ...
            {data && data.metrics_analyzed && (
              <div className="metric-grid">
                <div className="metric-item">
                  ...
                  <span className="metric-val" style={{ color: data.metrics_analyzed.temp > 85 ? 'var(--accent-3)' : '#fff' }}>
                    {data.metrics_analyzed.temp || '--'}
                  </span>
                </div>
                <div className="metric-item">
                ...
                <div className="metric-item">
                  ...
                  <span className="metric-val">{data.metrics_analyzed.network_bandwidth_gbps || '0'}</span>
                </div>
                <div className="metric-item">
                  <span className="metric-label">POWER LOAD</span>
                  <span className="metric-val">{data.metrics_analyzed.power_usage || '0'}</span>
                </div>
              </div>
            )}
